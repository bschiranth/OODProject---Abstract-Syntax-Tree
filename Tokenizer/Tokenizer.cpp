...
// AuthorChiranth Bangalore Sathyaprakash,Syracuse University				//
...
{
  if (!(_pContext->_pIn->good())) { return nullptr;}
  int chNext = _pContext->_pIn->peek();
  if (chNext == EOF) {
    _pContext->_pIn->clear(); }
  if (isOneCharToken(makeString(_pContext->currChar))){return _pContext->_pEatSpecialCharacters;}
  if (std::isspace(_pContext->currChar) && _pContext->currChar != '\n'){testLog("state: eatWhitespace");
 return _pContext->_pEatWhitespace;}
  if (_pContext->currChar == '/' && chNext == '/'){testLog("state: eatCppComment");
  ...
  if (_pContext->currChar == '\"' && _pContext->prevChar != '\\'){ return _pContext->_pEatDQString; }
  if (_pContext->currChar == '\'' && _pContext->prevChar != '\\'){return _pContext->_pEatSQString;}
  if (ispunct(_pContext->currChar)){
    testLog("state: eatPunctuator");
    return _pContext->_pEatPunctuator;}
  if (!_pContext->_pIn->good()){  return _pContext->_pEatWhitespace;}
